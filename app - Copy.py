...
def _generate_vouchers_page_html(vouchers: list, hotspot_login_url: str, include_print_button: bool = True) -> str:
    ...
    # Translations for fixed strings if they were to be dynamic (currently hardcoded in final HTML)
    # company_name_trans = _("The IT Dad")
    # contact_info_trans = _("Contact: +211 926 616 820")
    # email_info_trans = _("Email: justinkulang@gmail.com")
    # access_voucher_trans = _("Access Voucher")
    # username_trans = _("Username:")
    # password_trans = _("Password:")
    ...
    <head>
        ...
        <style>
            ...
            .voucher-container { 
                display: grid; 
                grid-template-columns: repeat(auto-fit, minmax(80mm, 1fr)); /* mm units */
                gap: 2mm; /* mm units */
                width: 100%; 
                max-width: 700px; /* Changed from 170mm to 700px */
            }
            .voucher { 
                background: white; 
                border: 1px solid #ccc; 
                border-radius: 10px; /* Kept px for subtle rounding, could be mm */
                padding: 2mm; /* mm units */
                page-break-inside: avoid; 
                box-shadow: 0 2px 5px rgba(0,0,0,0.05);
                width: 80mm; /* mm units */
                height: 60mm; /* mm units */
                display: flex;
                flex-direction: column; 
                justify-content: space-between; 
            }
            .voucher-details { /* No specific class changes needed here */ }
            .voucher-qr { 
                /* QR dimensions are set via inline styles on the div itself */
                display: flex;
                align-items: center;
                justify-content: center;
            }
            .voucher-qr img { max-width: 100%; max-height: 100%; }
            /* Branding, voucher-header, and credentials styles are primarily inline */
            .credentials span { 
                font-family: 'Courier New', monospace; 
                background: #f0f0f0; 
                padding: 1px 3px; /* px for fine control */
                border-radius: 4px; 
                color: #d63384; 
                font-weight: bold; 
            }
            @media print {
                body { margin: 0; padding: 0; background-color: #fff; -webkit-print-color-adjust: exact; print-color-adjust: exact; } /* Simplified margin for print */
    ...
    for voucher in vouchers:
        ...
        if QRCODE_AVAILABLE and hotspot_login_url:
            try:
                # Using box_size for QR code to fit new 20mm layout
                qr_code_b64 = generate_qr_code_base64(hotspot_login_url, username, password, box_size=5, border=1) # New box_size
            except Exception as e:
                logger.error(f"Error generating QR code for {username}: {e}")
        
        # Using f-string for cleaner HTML structure with mm/pt units
        voucher_html = f"""
        <div class="voucher">
            <div class="branding" style="text-align: center; padding-bottom: 1mm; border-bottom: 0.5pt solid #eee;">
                <h4 style="margin: 0 0 0.5mm 0; font-size: 11pt; color: #007bff;">The IT Dad</h4> 
                <p style="margin: 0.5mm 0; font-size: 8pt;">+211 926 616 820 | justinkulang@gmail.com</p>
            </div>
            <div class="voucher-content-row" style="display: flex; flex-direction: row; justify-content: space-between; align-items: center; flex-grow: 1;">
                <div class="voucher-details">
                    <div class="voucher-header" style="margin-bottom:1mm; padding-bottom:0.5mm;">
                        <h3 style="margin:0; font-size: 9pt;">{_('Access Voucher')}</h3>
                    </div>
                    <div class="credentials">
                        <p style="font-size: 8pt; margin: 0.5mm 0;"><strong>{_('Username:')}</strong> <span>{username}</span></p>
                        <p style="font-size: 8pt; margin: 0.5mm 0;"><strong>{_('Password:')}</strong> <span>{password}</span></p>
                    </div>
                </div>
                <div class="voucher-qr" style="width: 20mm; height: 20mm; margin-left: 1mm;">
        """
        html_parts.append(voucher_html)

        if qr_code_b64:
            alt_text = _('QR Code for {username}').format(username=username) # More natural translation
            html_parts.append(f'<img src="data:image/png;base64,{qr_code_b64}" alt="{alt_text}">')
        else:
            qr_na_trans = _("QR N/A")
            html_parts.append(f'<span style="font-size:0.8em; color:#aaa;">{qr_na_trans}</span>')
        
        html_parts.append("""
                </div>
            </div>
        </div>
        """) # Close voucher-qr, voucher-content-row, voucher
...
def generate_qr_code_base64(login_url, username, password, box_size=10, border=2):
    """Generates a QR code for login and returns a Base64 encoded string.
    Added box_size and border parameters for flexibility.
    """
    ...
    # Removed StyledPilImage and RoundedModuleDrawer for wider compatibility if not explicitly needed by user for styling.
    # If these specific stylings are essential, they can be kept.
    # For now, using default image_factory for broader compatibility.
    img = qr.make_image(fill_color="black", back_color="white")
    # If StyledPilImage is preferred:
...
def export_users_route():
    ...
    if export_format == 'json':
        ...
        <style>
            ...
            .voucher-details {{ /* No specific class changes needed here */ }}
            .voucher-qr {{{{ 
                /* Dimensions are set inline on the div */
                display: flex; 
                align-items: center; 
                justify-content: center; 
            }}}}
            .voucher-qr img {{{{ max-width: 100%; max-height: 100%; }}}}
            /* Branding, voucher-header, credentials, voucher-info are styled inline primarily */
            .credentials span {{{{ 
                font-family: 'Courier New', monospace; 
                background: #eee; 
                padding: 1px 3px; /* px for fine control */
                border-radius: 4px; /* px for consistency */
                color: #d63384; 
                font-weight: bold; 
            }}}}
            @media print {{{{ 
                body {{{{ margin: 0; background: #fff; -webkit-print-color-adjust: exact; print-color-adjust: exact; }}}} 
                .voucher {{{{ box-shadow: none; border: 1px dashed #999; }}}} 
            }}}}
        </style></head><body><div class="voucher-container">
        """
        for user in users:
            # Call generate_qr_code_base64 with new box_size for 20mm QR
            qr_code_b64 = generate_qr_code_base64(login_url, user.get('name'), user.get('password'), box_size=5, border=1) if login_url else None
            ...
            <div class="voucher">
                ...
                <div class="voucher-content-row" style="display: flex; flex-direction: row; justify-content: space-between; align-items: center; flex-grow: 1;">
                    <div class="voucher-details">
                        <div class="voucher-header" style="margin-bottom:1mm; padding-bottom:0.5mm;">
                            <h3 style="margin:0; font-size: 9pt;">{_('Hotspot Access')}</h3>
                        </div>
                        <div class="credentials">
                            <p style="font-size: 8pt; margin: 0.5mm 0;"><strong>{_('Username:')}</strong> <span>{user.get('name', 'N/A')}</span></p>
                            <p style="font-size: 8pt; margin: 0.5mm 0;"><strong>{_('Password:')}</strong> <span>{user.get('password') or 'N/A'}</span></p>
                        </div>
                        <div class="voucher-info" style="font-size: 8pt; margin-top: 1mm; border-top: 0.5pt solid #eee; padding-top: 1mm;">
            ...
            """
            if qr_code_b64:
                alt_qr_trans = _("QR Code for {username}").format(username=user.get('name', 'N/A'))
                html_content += f'<img src="data:image/png;base64,{qr_code_b64}" alt="{alt_qr_trans}">'
            else:
                qr_na_trans = _("QR N/A")
                html_content += f'<span style="font-size:0.8em; color:#aaa;">{qr_na_trans}</span>'
            
            html_content += """
                    </div> <!-- voucher-qr -->
                </div> <!-- voucher-content-row -->
            </div> <!-- voucher -->
            """
        html_content += """
